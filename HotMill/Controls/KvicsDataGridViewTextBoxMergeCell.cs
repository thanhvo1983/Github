...
namespace Kvics.Controls
{
    ...
    /// </summary>
    public class KvicsDataGridViewTextBoxMergeCell : DataGridViewTextBoxCell
    {
        #region Fields
        ...
        /// Default constructor.
        /// </summary>
        public KvicsDataGridViewTextBoxMergeCell()
            : base()
        {
            this.columnSpan = 1;
            this.rowSpan = 1;
        ...
        public KvicsDataGridViewTextBoxMergeCell(int rowSpan, int columnSpan, Color backColor, Color foreColor)
            : base()
        {
        ...
        public KvicsDataGridViewTextBoxMergeCell(int rowSpan, int columnSpan, Color backColor, Color foreColor, H_Align hAlign, V_Align vAlign)
            : base()
        {
            this.rowSpan = rowSpan;
            this.columnSpan = columnSpan;
        ...
        protected virtual void DataGridView_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            if (e.RowIndex >= this.RowIndex && e.RowIndex < this.RowIndex + this.rowSpan)
            {
                if (e.ColumnIndex >= this.ColumnIndex && e.ColumnIndex < this.ColumnIndex + this.columnSpan)
                {
                    ...
                    int stringY = displayRectangle.Y + 1;
                    ...
                    try
                    {
                        gridBorderPen = new Pen(this.DataGridView.GridColor);
                        foreBrush = new SolidBrush(this.foreColor);
                        backBrush = new SolidBrush(this.backColor);

                        e.Graphics.FillRectangle(backBrush, displayRectangle);
                        if (this.DataGridView.CellBorderStyle == DataGridViewCellBorderStyle.Single)
                        {
                            e.Graphics.DrawLine(gridBorderPen, displayRectangle.X - 1, displayRectangle.Top - 1, displayRectangle.Right - 1, displayRectangle.Top - 1);
                            e.Graphics.DrawLine(gridBorderPen, displayRectangle.Left - 1, displayRectangle.Y, displayRectangle.Left - 1, displayRectangle.Bottom - 1);

                            e.Graphics.DrawLine(gridBorderPen, displayRectangle.X - 1, displayRectangle.Bottom - 1, displayRectangle.Right - 1, displayRectangle.Bottom - 1);
                            e.Graphics.DrawLine(gridBorderPen, displayRectangle.Right - 1, displayRectangle.Y - 1, displayRectangle.Right - 1, displayRectangle.Bottom - 1);
                        }
